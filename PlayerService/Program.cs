...
// Add EF Core with SQL Server (Azure SQL)
builder.Services.AddDbContext<PlayerDb>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection") ?? "Server=azuresql;Database=PlayerDb;User Id=sa;Password=Your_password123;TrustServerCertificate=True;"));
...
{
    var db = scope.ServiceProvider.GetRequiredService<PlayerDb>();
    var maxRetries = 10;
    var delayMs = 3000;
    var retries = 0;
    bool dbReady = false;
    string dbName = "PlayerDb";
