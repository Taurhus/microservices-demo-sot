...
builder.Services.AddDbContext<ItemDb>(options =>
    options.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection") ?? "Server=azuresql;Database=ItemDb;User Id=sa;Password=Your_password123;TrustServerCertificate=True;"));
...
// Apply migrations at startup with wait/retry for DB connectivity and create DB if missing
{
    ...
    var db = scope.ServiceProvider.GetRequiredService<ItemDb>();
    var maxRetries = 10;
    var delayMs = 3000;
    var retries = 0;
    bool dbReady = false;
